...
- **Teknisyen Çözüm Kontrolü & Renk Dengesi:**
  - Teknisyen düzenleme sayfasında çözüm notu girilmesine rağmen durumun 'çözüldü' seçilmediği durumlar için akıllı kontrol eklendi: Ekranda onay kutusu (popup) gösterilerek 'çözüldü' durumu otomatik atanıp kaydedilmesi veya iptal edilmesi sağlanır.
  - Turuncu (Amber) butonlardaki beyaz yazı rengi özelleştirmesi, açık modda (light mode) varsayılan görünümün bozulmaması için sadece karanlık modda (dark mode) aktif olacak şekilde (.dark sınıf ön ekiyle) sınırlandırıldı.
- **Faaliyet Yetkilendirmesi & Görünüm Sayfası:**
  - Bir faaliyet kaydının (Activity) sadece onu oluşturan/açan kullanıcı tarafından silinebilmesi veya düzenlenebilmesi için `ActivityPolicy` güncellendi. Başkasının faaliyetine tıklandığında düzenleme (Edit) sayfası yerine salt-okunur detay (View) sayfası açılır, kendi faaliyeti olduğunda ise düzenleme sayfası açılır.
- **Faaliyet Sayfa Başlığı Kısaltma:**
  - Detay sayfasında ve ekmek kırıntısı (breadcrumbs) alanında başlık olarak faaliyet açıklaması (description) kullanıldığında oluşan aşırı uzun görsel kirliliği engellemek için, başlık `Faaliyet Türü (Tarih)` (örneğin: `Telefon Desteği (29.06.2026)`) şeklinde optimize edildi.
- **Talepleri Aktarma ve Personel Birleştirme (Artisan):**
  - İhtiyaç fazlası veya mükerrer personellerin tüm taleplerini (atanan ve çözülen) ve faaliyetlerini başka bir personele güvenle aktarıp eski kullanıcıyı silebilen `tickets:transfer {from} {to}` Artisan komutu geliştirildi.
- **Raporlardan Taleplere Doğrudan Erişim:**
  - Teknisyen Raporları sayfasında listelenen kapatılan taleplerin "Takip No" alanlarına tıklanabilir link özelliği eklendi. Tıklayan yetkili, talebi yeni sekmede (`target="_blank"`) açarak doğrudan detayına erişebilir.
- **Admin Panel Dashboard (Panel) Yenileme:**
  - Tüm eski widget'lar (TicketStatsOverview, MyActiveTickets, TechnicianPerformanceTable, ActiveAnnouncements, AssetsChart) kaldırılıp tek bir `DashboardOverview` widget ile değiştirildi.
  - 5 renkli gradyan istatistik kartı: Toplam Talep, Bekleyen, Bugün Gelen, Çözülen, Acil & Bekleyen.
  - Acil bekleyen talep varsa kırmızı uyarı bandı gösterilir; tıklanınca filtreli talep listesine yönlendirir.
  - Aktif duyurular (Announcements) panelin üstünde renkli badge'lerle listelenir.
  - Bekleyen talepler: tıklanabilir takip no, önceliğe göre renkli nokta/badge, bölüm bilgisi.
  - Son 7 günlük trend grafiği: mor (gelen) ve yeşil (çözülen) barlar yan yana; haftalık özet + çözüm oranı %.
  - Teknisyen performans tablosu: avatar, aktif iş / bugün çözülen / toplam çözülen / progress bar (sadece admin).
  - `App\Filament\Pages\Dashboard` özel sınıfı oluşturuldu → `getColumns()=1` ile tam genişlik (`screen-2xl`).
  - `$isLazy = false` eklenerek lazy loading devre dışı bırakıldı (boş ekran sorunu giderildi).
  - Anasayfa nav linki "Ayarlar" grubuna alınarak menünün en sonuna yerleştirildi.
